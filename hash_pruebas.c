...
void prueba_mateo(){
    ...
    Prueba hash iterador esta al final, es true... OK
    Se crea nuevo iterador, que sera destruido sin llegar al final (posible memory leak)... OK
    Insertar tres valores... OK
    Borrar los tres valores... OK
    El iterador creado tras los borrados está al final... OK
    Ver actual del iterador tras los borrados es NULL... OK
    Se inserta un nuevo valor... OK
    Tras insertar, un nuevo iterador no está al final... OK
    Tras insertar, su ver actual es la clave insertada... OK
    Tras llamar a avanzar, el iterador sí está al final: ERROR------------------------
    pruebas_catedra.c:488: hash_iter_al_final(iter)
    Prueba hash iterador esta al final, es false... OK*/

    printf("PRUEBAS MATEO................\n");
    hash_t* hash = hash_crear(free);

    /*char *claveA = "AAA", *valorA = "aaa";
    char *claveB = "BBB", *valorB = "bbb";
    char *claveC = "CCC", *valorC = "cc";
    print_test("Prueba hash insertar clave AAA", hash_guardar(hash, claveA, valorA));
    print_test("Prueba hash insertar clave BBB", hash_guardar(hash, claveB, valorB));
    print_test("Prueba hash insertar clave CCC", hash_guardar(hash, claveC, valorC));
    hash_iter_t* iterA = hash_iter_crear(hash);
    while(!hash_iter_al_final(iterA)){
        print_test("Prueba hash iterador esta al final, es false...", !hash_iter_al_final(iterA));
        print_test("Prueba hash iterador ver actual, es una clave valida...", hash_iter_ver_actual(iterA));
        const char* actual = hash_iter_ver_actual(iterA);
        printf("actual es %s\n", actual);
        print_test("Prueba hash iterador avanzar es true...", hash_iter_avanzar(iterA));
    }
    print_test("Prueba hash iterador esta al final, es true...", hash_iter_al_final(iterA));
    print_test("Prueba hash iterador ver actual, es NULL...", !hash_iter_ver_actual(iterA));
    print_test("Prueba hash iterador avanzar es false...", !hash_iter_avanzar(iterA));
    print_test("Prueba hash iterador esta al final, es true...", hash_iter_al_final(iterA));
    hash_iter_destruir(iterA);*/

    char *clave1 = "perro", *valor1 = "guau";
    char *clave2 = "gato", *valor2 = "miau";
    char *clave3 = "vaca", *valor3 = "mu";

    print_test("Prueba hash insertar clave perro", hash_guardar(hash, clave1, valor1));
    print_test("Prueba hash insertar clave gato", hash_guardar(hash, clave2, valor2));
    print_test("Prueba hash insertar clave vaca", hash_guardar(hash, clave3, valor3));
    hash_borrar(hash, clave1);
    hash_borrar(hash, clave2);
    hash_borrar(hash, clave3);
    printf("borre todos\n");
    hash_iter_t* iter = hash_iter_crear(hash);
    print_test("El iterador creado tras los borrados está al final...", hash_iter_al_final(iter));
    print_test("Ver actual del iterador tras los borrados es NULL...", !hash_iter_ver_actual(iter));
    char *clave4 = "oveja", *valor4 = "mee";
    print_test("Se inserta un nuevo valor...", hash_guardar(hash, clave4, valor4));
    hash_iter_destruir(iter);

    //hash_iter_t* iter2 = hash_iter_crear(hash);
    //print_test("Tras insertar, un nuevo iterador no está al final...", !hash_iter_al_final(iter2));
    //printf("Ver actual es ...... %s\n", hash_iter_ver_actual(iter2));

    //hash_iter_t* iter3 = hash_iter_crear(hash);
    //char *clave5 = "chajá", *valor5 = "chajá";
    //print_test("Inserté clave nueva", hash_guardar(hash, clave5, valor5));
    //hash_iter_destruir(iter2);
    //print_test("Tras insertar, su ver actual es la clave insertada...", hash_iter_ver_actual(iter2) == clave5);
    //printf("ver actual entonceees es %s\n", hash_iter_ver_actual(iter2));
    //hash_iter_avanzar(iter2);
    //print_test("Tras llamar a avanzar, el iterador sí está al final", hash_iter_al_final(iter2));
    //print_test("Prueba hash iterador esta al final, es false...", hash_iter_al_final(iter2));
